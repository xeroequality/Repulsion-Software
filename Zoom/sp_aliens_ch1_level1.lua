...
function scene:createScene( event )
		...
		background = display.newImageRect(group,"../images/background_chapter1_level1_background.png",1500,1000);
		background.x = 0; background.y = 0;
		city = display.newImageRect(group,"../images/background_chapter1_level1_foreground.png",1500,200);
		city.x = 0; city.y = 220;
		city2 = display.newImageRect(group,"../images/background_chapter1_level1_foreground.png",1500,200);
		city2.x = 1500; city2.y = 220;
...
function scene:enterScene( event )
		...
		local CENTER = {745,display.contentHeight*0.5}; --Good Overlay.width, Center of Height Screen
		...
		local zoom_scale = 1; --How Much the Zoom Changes when... Zooming
		local z,s,disx,disy;
		...
		function Zoom_Out(event)
			...
			if event.phase == "began" then
				...
				z = zoom-(zoom_scale); --The New Zoom Value
				for i=2,group.numChildren do
					local child = group[i]
					if child.zoomy == nil then
						--Adjust the X and Y Based on CENTER
						--Get It's Distance from the Center
						--Get Original X First
						--s = (100-zoom)/100;
						--cx = (child.x-(s*CENTER[1]))/(1-s); cy = (child.y-(s*CENTER[2]))/(1-s);
						disx = CENTER[1]-child.x;
						disy = CENTER[2]-child.y;
						disx = disx*(zoom_scale/100); disy = disy*(zoom_scale/100);
						child.y = child.y + disy;
						child.x = child.x + disx;
						child.xScale = (z/100);
						child.yScale = (z/100);
					end
				end
