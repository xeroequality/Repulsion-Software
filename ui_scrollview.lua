...
function scene:enterScene( event )
		...
		----------------------------------------------
		--Make Them Cannons
		local cannon = display.newImage(group,"../images/cannon_sm.png");
		local cannon_base = display.newImage(group,"../images/cannon_base_sm.png");
		local crosshair = display.newImage(group,"../images/target.png");
		cannon_base.x = 200; cannon_base.y = H-10-35;
		cannon.x = 230; cannon.y = H-10-70;
		crosshair.alpha = 0; crosshair:scale(0.5,0.5);
		...
		--Rotate Them Crosshair
		local function rotateCrosshair(event)
			crosshair:rotate(4);
		end
		
		--Cannon Blasting
		--Touching the Cannon
		local function touchCannon(event)
			if showTarget == false then
				showTarget = true;
				crosshair.x = cannon.x + 50;
				crosshair.y = cannon.y - 100;
				cannonAngle = 0;
				crosshair.alpha = 1;
			end
		end
		...
		local line = display.newLine(group,0,0,0,0);
		line.alpha = 0.01;
		local h,b,a,pheta,rpheta,newMove;
		local r = 100;
		function targetingSystem(event)
			if event.phase == "began" and showTarget == true then
				display.getCurrentStage():setFocus( crosshair )
				crosshair.isFocus = true
				print("here")
			end
			if showTarget == true then
				--Create the Line
				--Get Pheta
				--[[Hypotenuse
				h = math.sqrt( ((event.x-crosshair.x)^2) + ((event.y-crosshair.y)^2) );
				--Opposite
				b = math.sqrt( (event.x-crosshair.x)^2);
				a = (event.y-crosshair.y)
				--Pheta
				if h == 0 then h = 0.0001 end
				pheta = math.asin(b/h);
				pheta = (2*math.pi)-pheta;
				--Degrees
				apheta = pheta * (180/math.pi);
				--print(apheta)--]]
				
				--Draw the Line
				line:removeSelf();
				line = display.newLine(group,crosshair.x,crosshair.y,event.x,event.y);
				print("Still Here");
				--Anglize the Cannon
		...
		--The Listeners
		Runtime:addEventListener("enterFrame",rotateCrosshair);
		cannon_base:addEventListener("touch",touchCannon);
		crosshair:addEventListener("touch",targetingSystem);
